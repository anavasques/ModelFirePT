...
%             StoreMatPine(NrStore)= MatPine;
 ...
 %%%%%% MULTIRUNS CODE
        filename=strcat(['firePT_NOF_MAXSS_OS',num2str(maxseedSeed(k)),'_',num2str(irun),'.mat' ]);
...
%         save(filename,'VectorTime','StorePine','StoreSeeder','StoreOak')
%         %     filename=strcat(['seedstart',num2str(maxseedSeed(k)),'.mat' ]);
%         %     save(filename,'matr','-ascii') %save a text file
%     %%%Plotting over time
%     figure
...
%     ylabel ('Cover (%)');
%     title(filename)
%     filename=strcat(['nofire_maxseed',num2str(maxseedSeed(k)),'_',num2str(irun),'.tif' ]); %saves the files
