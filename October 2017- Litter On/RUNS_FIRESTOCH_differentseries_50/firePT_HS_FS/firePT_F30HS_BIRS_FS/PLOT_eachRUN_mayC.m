...
%pineTimeM=zeros(nruns,length(maxseedSeed));pineTimestd=pineTimeM;
    ...
    for k=1:length(b)
     figure(10)
        for irun=1:nruns
       filename=strcat(['firePT_F30HS_BIRS_FS5','_',num2str(irun),'_',num2str(b(k)),'.mat' ]);%loads the file in the current directory %%name of directory should correspond; can load many directories
