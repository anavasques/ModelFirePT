...
storeMatrFS0=zeros(length(F)*length(b)*nruns,6);
storeMatrAvFS0=zeros(length(F)*length(b),6);
iii=0;
ii=0;
...
for i=1:length(BirdSeedNv) % fire recurrence 
    foldername= strcat('firePT_', num2str(BirdSeedNv(i)),'F0_BIRS_FS'); %if file exists
...
% figure(i)
    ...
    for k=1:length(b)
       ...
       filename=strcat(['firePT_F0_BIRS_FS',num2str(BirdSeedNv(i)),'_',num2str(irun),'_',num2str(b(k)),'.mat' ]);%loads the file in the current directory %%name of directory should correspond; can load many directories
...
save storeMatrFS0.txt storeMatrFS0 -ASCII
save storeMatrAvFS0.txt storeMatrAvFS0 -ASCII
