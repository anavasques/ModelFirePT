...
for k=1:length(BirdSeedNv)
    figure (10)   
    for irun=1:nruns
        filename=strcat(['./firePT_F7LS_BIRS_OS',num2str(BirdSeedNv(k)),'_',num2str(irun),'_122','.mat' ]);%loads the file in the current directory %%name of directory should correspond; can load many directories
        load (filename)% command to load only certain variables: load(filename,variables) e.g. %'StorePine','StoreSeeder','StoreOak','StoreLitter','VectorTime')
         %here we can put any number, but a number should be in the parenthesis for it to open always the same figure and plot all in the same figure
        
        % One shade of blue red or green for each veg type and each seed number: 
        plot (VectorTime,StorePine/m/m*100,'Color',[0 0+k/length(BirdSeedNv) 1])
        hold on % hold on should be after the plot
        plot(VectorTime,StoreSeeder/m/m*100,'Color',[1 0 ((k-1)/(length(BirdSeedNv)-1))])
        plot (VectorTime, StoreOak/m/m*100,'Color',[0 k/length(BirdSeedNv) 0])
        %Note to self (MAra) for another way of changin colors:'Color',[0+k/length(BirdSeedNv) 0+sin(k/length(BirdSeedNv)*3.14) 1-k/length(BirdSeedNv)])
