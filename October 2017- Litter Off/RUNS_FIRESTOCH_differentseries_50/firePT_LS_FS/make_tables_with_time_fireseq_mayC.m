...
for i=1:length(F) % fire recurrence 
    foldername=strcat('firePT_F',num2str(F(i)),'LS_BIRS_FS'); %if file exists
...
% figure(i)
    ...
    for k=1:length(b)
       ...
       filename=strcat(['firePT_F',num2str(F(i)),'LS_BIRS_FS1','_',num2str(irun),'_',num2str(b(k)),'.mat' ]);%loads the file in the current directory %%name of directory should correspond; can load many directories
