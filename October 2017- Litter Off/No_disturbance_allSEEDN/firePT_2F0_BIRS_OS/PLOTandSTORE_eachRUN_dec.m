...
b=122; %%%%%HERE only for one fire series
%pineTimeM=zeros(nruns,length(maxseedSeed));pineTimestd=pineTimeM;
% storeRUNSMatr=zeros(length(b)*nruns*3000,8);% not used
% iii=0;

    for k=1:length(b)
     figure(10)
        for irun=1:nruns
       filename=strcat(['firePT_F0_BIRS_FS',num2str(BIRS),'_',num2str(irun),'_',num2str(b(k)),'.mat' ]);%loads the file in the current directory %%name of directory should correspond; can load many directories
...
%        else
           load (filename)
        plot(VectorTime, StoreLitter/m/m, 'g--')
        hold on
        set(gca,'fontsize',14, 'fontWeight','bold');
        set(gcf,'Position',[2 2 750 500],'PaperPositionMode','auto');
        set(gca,'fontsize',16, 'fontWeight','bold');
        xlabel('Time (year)');
        ylabel ('Average litter depth per cell (cm)');
        axis([0 2000 0 10]);
        end
        end
