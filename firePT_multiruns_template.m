...
% here we can change the values that will be used for the repetitions of the
% runs 
...
%pd=3:10:43
b= [122:1:131]; % repetitions for the fire series (each number is a random number that corresponds to a fire sequence)
...
while tf<EndTime% EndTime can be substituted for the time when disturbance should stop
    ...
    D(round(tf))=1;
        ...
        while Time < EndTime
            Time= Time+dt;
            %%%% Creates LITTER in the neighbourhood of pine (8 neighbours)+ pine plce itself
            ...
            for i = 1 : m
                for j=1:m           % for all the cells of the lattice
                    ...
                    if TC(i,j)==0   % empty cells
                        ...
                        % be <than 3 and we don't need to multiply it by dt
                        % anymore
...
%%%% DISTURBANCE
 % NOTE - the rest of the disturbance term is at the beggining of the code
    D1=D(Time);
    if D1== 1
        'fire'; % can be written in the comand window when a fire occurs
        ...
        %neightbours; we did various experiments with this so that the
        %litter layer would be reallistic in a mature pine forest(~4-5 cm after the fire)
        for i=1:m
            for j=1:m
