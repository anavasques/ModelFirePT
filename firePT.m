...
%filename=strcat(['par',num2str(r),'.mat']);
%save(filename)
...
while tf<EndTime % EndTime can be substituted for the time when disturbance should stop
    %%%%%MULTI%%%%%%%%%%%%%%%Multiruns for fire return (SEE WITH MARA)
...
%         Lit(:,:)=0;
    ...
    %%% store for a period of 100 or 150 years for e.g.
    %PeriodTime(NrStore==100)= Time
    NrStore = NrStore+1;
    StoreTime = StoreStep;
