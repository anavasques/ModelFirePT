...
SB=[0*m*m 100*m*m 0+randi(BirdSeedN,1)]; %initial seed bank %comment this on the multiruns
...
while Time < EndTime
    ...
    %%%if TC(Age>AgeMP)==1 %if in the first years pine do not create litter
        [x,y]=find(TC(2:end-1,2:end-1)==1); %finds cells =1 in the whole matrix
        x=x+1;y=y+1;
        for i=1:length(x)
    ...
    for i = 1 : m
        for j=1:m
            ...
            if TC(i,j)==0 % colonization/germination
                ...
                % version Mara i.e. prob only needs to be <than 3 and we don't multiply by dt anymore)
