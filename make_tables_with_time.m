...
for i=1:length(F) % fire recurrence 
    foldername=strcat('firePT_F',num2str(F(i)),'HS_BIRS_FS');
    ...
    for k=1:length(BirdSeedNv)
        ...
        for irun=1:nruns
         iii=iii+1;
            filename=strcat(['firePT_F',num2str(F(i)),'HS_BIRS_FS',num2str(BirdSeedNv(k)),'_',num2str(irun),'.mat' ]);%loads the file in the current directory %%name of directory should correspond; can load many directories
...
PinePerc=StorePine/100/100*100;
OakPerc=StoreOak/100/100*100;
            vec=VectorTime(PinePerc==0);
            if isempty(vec)
...
%     set(gca,'xtick',1:4,'xticklabel',[' 0 '; ' 7 '; ' 15 '; ' 30 '],'ylim',[0 500])
%     figure(100+k)
