...
storeMatr=zeros(length(F)*length(b)*nruns,5);
storeMatrAv=zeros(length(F)*length(b),6);
iii=0;
ii=0;
...
<<<<<<< Updated upstream:RUNS May 16 (now working) Maras computer/make_tables_with_time_fireseq_mayC.m
...
figure(i)
=======
for i=2:2%1:length(F) % fire recurrence 
    foldername=strcat('firePT_F',num2str(F(i)),'HS_BIRS_FS'); %if file exists
    % I don't know your foldername but you can here use the FR(i) to create
    % your folder name

>>>>>>> Stashed changes:make_tables_fireseq/firePT_HS_FS/make_tables_with_time_fireseq.m
...
<<<<<<< Updated upstream:RUNS May 16 (now working) Maras computer/make_tables_with_time_fireseq_mayC.m
       ...
       filename=strcat(['firePT_F',num2str(F(i)),'HS_BIRS_OS5','_',num2str(irun),'_',num2str(b(k)),'.mat' ]);%loads the file in the current directory %%name of directory should correspond; can load many directories
...
%        else
           ...
           plot(VectorTime,StorePine,VectorTime,StoreSeeder,VectorTime,StoreOak)% plots all the runs - one figure for each fire condition
           hold on
           %pause
...
=======
            filename=strcat(['firePT_F',num2str(F(i)),'HS_BIRS_FS5','_',num2str(irun),'_',num2str(b(k)),'.mat' ]);%loads the file in the current directory %%name of directory should correspond; can load many directories
