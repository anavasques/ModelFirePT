...
BirdSeedN=5;              % Annual seed input by birds in the whole laticce [n/year] % average values Q. suber Pons and Pausas 2007 50 seeds per hectar - this value depends on surrounding populations
...
BirdSeedNv=5; % MARA MAY 17TH 2016 this should be a vector for experiments with different seed numbers
...
nruns=100;                % number of repetitions of each run (exactly the same conditions)


%%%% VECTOR FOR MULTIPLE RUNS WITH DIFFEREENT NUMBER OF SEEDS
% NB MARA MAY 17TH 2016, THIS WAY IT WORKS ALSO IF BIRDSEEDNV IS JUST A
% NUMBER NOT A VECTOR
% oak
for k=1:length(BirdSeedNv)
    ...
    for b=122:131; % repetitions for the fire series (each number is a random number that corresponds to a fire sequence)
        ...
        rng(b); % INITIALISE THE RAND COMMANDS TO OBTAIN A SPECIF FIRE SEQUENCE; MARA MAY 17TH 2016:I CHANGED THIS TO RNG AS THE OTHER WAY OF WRITING WAS OLD-FASHIONED
        ...
        for irun=1:nruns;
            ...
            while Time < EndTime
                ...
                for kk=1:SB(3)      %only happens if SB3 is EQUAL OR bigger than 1 % MARA May17 2016: IT HAPPENS ALSO FOR 1 SEED ONLY (BUT THAT' OK, ISN'T IT?)
%                     cc=randi(m,1,2);
%                     PosQSeed(cc(1),cc(2))=PosQSeed(cc(1),cc(2))+1;
        ...
        %     %%%%%% FIGURES OF THE MULTIRUNS(INCLUDED IN SEPARATE FILES)
        %     figure
        %     plot(VectorTime,mPineMR/m/m*100,'b', VectorTime,mSeederMR/m/m*100, 'r--.', VectorTime,mOakMR/m/m*100, 'g*')%, VectorTime,StoreLitter/m/m, 'k.')%, VectorTime,StoreAge,'gr')
